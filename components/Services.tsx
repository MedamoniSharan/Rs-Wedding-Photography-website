...
const ServiceCard: React.FC<{ service: Service; className?: string }> = ({ service, className }) => {
  return (
    <div className={className}>
      ...
      <div className="mb-4">
        <img src={service.imageSrc} alt={service.title} className="w-full object-cover rounded-2xl shadow-lg aspect-[4/3]" />
      </div>
      <p className="text-gray-600 dark:text-gray-400 mb-6 text-left">{service.description}</p>
      <a href="#" className="w-14 h-14 border border-gray-300 dark:border-gray-700 rounded-full flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-charcoal-gray dark:hover:bg-gray-700 hover:text-white dark:hover:text-white transition-colors duration-300" aria-label={`Learn more about ${service.title}`}>
...
const Services: React.FC = () => {
  return (
    <section className="py-16 md:py-24 bg-[#F6F4F1] dark:bg-charcoal-gray/50">
